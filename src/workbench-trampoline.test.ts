...
describe("relayExitCode", () => {
  ...
  it("forwards a relay's exit status", () => {
    const io = fakeIo({ probeResult: { status: 0 }, relayOutcome: { status: 0, signal: null } });

    expect(relayExitCode({ status: 9, signal: null }, io)).toBe(9);
  });

  it("reports missing Bun when the relay failed to launch", () => {
    const io = fakeIo({ probeResult: { status: 0 }, relayOutcome: { status: 0, signal: null } });

    expect(relayExitCode({ error: { message: "x" }, status: null }, io)).toBe(
      MISSING_BUN_EXIT_CODE
    );
  });
});
